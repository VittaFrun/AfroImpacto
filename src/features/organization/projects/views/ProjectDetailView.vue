      ...
      <v-row>
        ...
        <v-col cols="12" md="8">
          ...
          <v-card class="pa-6 rounded-lg elevation-0 mb-6">
            ...
            <kanban-board :tasks="project.tasks" :phases="project.phases"
              @update:taskStatus="updateTaskStatusFromKanban" @task-clicked="openEditTaskDialog"></kanban-board>
          </v-card>
              ...
              <v-expansion-panel v-for="phase in project.phases" :key="phase.id">
                <v-expansion-panel-title>
                  <div class="d-flex justify-space-between align-center w-100">
                    ...
                    <div>
                      ...
                      <v-btn icon="mdi-pencil" variant="text" color="warning" size="small"
                        @click.stop="openEditPhaseDialog(phase)"></v-btn>
                      <v-btn icon="mdi-delete" variant="text" color="error" size="small"
                        @click.stop="confirmDeletePhase(phase.id)"></v-btn>
                    </div>
                  </div>
                </v-expansion-panel-title>
                <v-expansion-panel-text>
                  ...
                  <v-list density="compact">
                    <div v-for="task in tasksByPhase(phase.id)" :key="task.id">
                      <v-list-item>
                        ...
                        <v-list-item-subtitle>
                          ...
                          <span v-if="task.assignments.length">Asignado a: <span
                              v-for="(assignment, idx) in task.assignments" :key="assignment.assignmentId">{{
                                assignment.volunteerName }} ({{ assignment.role }})<span
                                v-if="idx < task.assignments.length - 1">, </span></span></span>
                        </v-list-item-subtitle>
                        <template v-slot:append>
                          <v-btn icon="mdi-pencil" variant="text" color="warning" size="x-small"
                            @click.stop="openEditTaskDialog(task)"></v-btn>
                          <v-btn icon="mdi-delete" variant="text" color="error" size="x-small"
                            @click.stop="confirmDeleteTask(task.id)"></v-btn>
                        </template>
                      </v-list-item>
                    </div>
                    ...
                    <v-list-item>
                      <v-btn block variant="tonal" color="primary" @click="openAddTaskDialog(null, phase.id)">+ Añadir
                        Tarea a esta Fase</v-btn>
                    </v-list-item>
                  </v-list>
                </v-expansion-panel-text>
              </v-expansion-panel>
        ...
        <v-col cols="12" md="4">
          ...
          <v-card class="pa-6 rounded-lg elevation-0 mb-6">
            ...
            <v-divider class="mb-4"></v-divider>
            <v-btn color="info" block class="mb-3" @click="manageRolesDialog = true"><v-icon
                start>mdi-account-cog</v-icon>Gestionar Roles</v-btn>
            <v-btn color="success" block @click="openAssignVolunteerDialog"><v-icon
                start>mdi-account-plus</v-icon>Asignar Voluntario a Tarea</v-btn>
          </v-card>
          ...
          <v-card class="pa-6 rounded-lg elevation-0 mt-6">
            ...
            <v-timeline density="compact" side="end">
              <v-timeline-item v-for="(activity, index) in recentActivities" :key="index" :dot-color="activity.color"
                :icon="activity.icon" size="small">
                <div class="d-flex justify-space-between flex-grow-1">
                  <div>{{ activity.description }}</div>
                  <div class="text-caption text-grey">{{ formatDistanceToNow(activity.timestamp, { addSuffix: true,
                    locale: es }) }}</div>
                </div>
              </v-timeline-item>
              <v-card-text v-if="recentActivities.length === 0" class="text-center grey--text">No hay actividad
                reciente.</v-card-text>
            </v-timeline>
          </v-card>
        </v-col>
      </v-row>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form @submit.prevent="assignVolunteerToTask">
            <v-select v-model="selectedTaskForAssignmentId" :items="project.tasks" item-title="description"
              item-value="id" label="Seleccionar Tarea" outlined dense class="mb-4"></v-select>
            <v-select v-model="selectedVolunteerId" :items="availableVolunteers" item-title="name"
              item-value="id_voluntario" label="Seleccionar Voluntario" outlined dense class="mb-4"></v-select>
            <v-select v-model="assignedRole" :items="customRoles" item-title="name" item-value="name"
              label="Rol Asignado" outlined dense class="mb-4"></v-select>
...
function savePhase() {
  if (currentPhase.value) {
    projectStore.updateProjectPhase(project.value.id, phaseForm.value.id, phaseForm.value);
...
const saveTask = () => {
  ...
  if (currentTask.value) {
    ...
    projectStore.updateProjectTask(project.value.id, currentTask.value.id, taskForm.value);
...
const saveProjectChanges = () => {
  if (projectForm.value) {
    // Assuming updateProject can handle simple object updates for non-file changes
    projectStore.updateProject(projectForm.value.id, projectForm.value);
...
const updateTaskStatusFromKanban = ({ taskId, newStatus }) => {
  ...
  if (taskToUpdate) {
    projectStore.updateProjectTask(project.value.id, taskToUpdate.id, { ...taskToUpdate, status: newStatus });
