      ...
      <v-row class="mb-6">
        ...
        <v-col cols="12" md="6">
          <v-card class="pa-6 rounded-lg elevation-0 highlight-card h-100">
            ...
            <div v-if="mainProject">
              <v-row>
                <v-col cols="12" md="4">
                  <v-img :src="mainProject.coverImage" class="rounded-lg" height="150" cover></v-img>
                </v-col>
                <v-col cols="12" md="8">
            ...
            <div v-else>
              <p class="text-body-1 text-grey-darken-1">No hay proyectos o no se ha seleccionado un proyecto principal.</p>
            </div>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="6">
          <v-card class="pa-6 rounded-lg elevation-0 highlight-card h-100">
            ...
            <v-divider class="mb-4"></v-divider>
            <div v-if="mainProject && mainProject.tasks && mainProject.tasks.length > 0">
              <v-timeline density="compact" side="end" class="timeline-custom">
                <v-timeline-item v-for="task in mainProject.tasks" :key="task.id" :dot-color="task.statusColor"
                  size="small">
            ...
            <div v-else>
              <p class="text-body-1 text-grey-darken-1">No hay tareas definidas para el proyecto principal.</p>
            </div>
          </v-card>
        </v-col>
      </v-row>
      ...
      <v-row>
        ...
        <v-col cols="12" md="8">
          <v-card class="pa-6 rounded-lg elevation-0">
            ...
            <v-divider class="mb-4"></v-divider>
            <v-data-table :headers="headers" :items="projects" item-value="id" class="elevation-0 projects-table">
              ...
              <template v-slot:item.progress="{ item }">
                <div class="d-flex align-center">
                  ...
                  <v-progress-linear :model-value="item.progress.percentage" :color="item.statusColor" height="8"
              ...
              <template v-slot:item.main="{ item }">
                <v-btn icon @click="projectStore.setMainProject(item.id)">
                  <v-icon>{{ mainProject && item.id === mainProject.id ? 'mdi-star' : 'mdi-star-outline' }}</v-icon>
                </v-btn>
              </template>
            </v-data-table>
          </v-card>
        ...
        <v-col cols="12" md="4">
          <v-card class="pa-6 rounded-lg elevation-0 highlight-card">
            ...
            <div v-if="mainProject && projectProgress.phases.length > 0">
              ...
              <v-timeline density="compact" side="end" class="timeline-custom">
                <v-timeline-item v-for="(phase, index) in projectProgress.phases" :key="phase.id"
                  :dot-color="phase.statusColor" size="small">
                  <div class="d-flex justify-space-between flex-grow-1">
                    <div>
              ...
              <v-card-actions class="pa-0 mt-4">
                <v-btn variant="text" color="primary" @click="viewProjectDetails(mainProject.id)">Gestionar
                  proyecto</v-btn>
              </v-card-actions>
            </div>
            <div v-else>
...
const projectProgress = computed(() => {
  if (!mainProject.value || !mainProject.value.phases) return { phases: [] };
